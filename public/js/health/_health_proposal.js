...
$('body').on('click','.btn-next-address',function(e){
    ...
    if ($("#addressInfo-Form").valid() === true) {
        var isDocidValid =docIdvalid($('#documentId'),$('#documentType').val(),$('#documentId').val());
        if(isDocidValid){
             ...
             var _document = {}
                ...
                _document.documentId   = $('#documentId').val(); 
       ...
       if(hasMedical=='YES'){
           if(checkedNum){
               $(".medical_que" ).each(function( index ) {
                  if($(this).is(":checked")){
                    var queId =  $(this).val();
                    var isBoolean =  ($(this).attr('data-boolean')=='YES')?true:false;
                    var isText    =  ($(this).attr('data-text')=='YES')?true:false;
                    var isSince   =  ($(this).attr('data-since')=='YES')?true:false;
                    var queTitle  =  $(this).attr('data-title');
                       if(questionsArr[queId]){ questionsArr[queId].status = true;}
                       else{questionsArr[queId] = {queid:queId,status:true } }
                    ...
                    if(usercheckedNum){
                         ...
                         $(".ins-members-"+queId ).each(function( index ) {
                            if($(this).is(":checked")){
                                 ...
                                 var questions = {};
                                 questions = (healthInfo.members[mkey].medical)?healthInfo.members[mkey].medical:questions;
                                 questions[queId]= {queid :queId ,bool :'YES'};
                                 //questions[queId].bool = 'YES';
                                 if(isSince && ($('#medi_since_mm_'+q_uid).length>0)){
                                     var mm = $('#medi_since_mm_'+q_uid).val();
                                     var yy = $('#medi_since_yy_'+q_uid).val();
                                     if(mm!=="" && yy!==""){
                                        $('.error-time-since-'+q_uid).text("");
                                         //var questions = {};
                                         //questions = (healthInfo.members[mkey].medical)?healthInfo.members[mkey].medical:questions;
                                        if(questions[queId]){
                                            questions[queId].since = mm+"-"+yy;
                                        }else{
                                           questions[queId] = {queid:queId,since:mm+"-"+yy }
                                        }
                                        
                                        healthInfo.members[mkey].medical = questions;
                                     }else{
                                        error++;
                                        $('.error-time-since-'+q_uid).text("Since when "+username+" suffering  from "+queTitle);
                                     }
                                 }
                                //  if(isBoolean && ($('#medi_boolean_'+q_uid).length>0)){
                                //      var bool = $('#medi_boolean_'+q_uid).val();
                                //      if(bool!==""){
                                //         $('.error-yesno-boolean-'+q_uid).text("");
                                //         var questions = {};
                                //          questions = (healthInfo.members[mkey].medical)?healthInfo.members[mkey].medical:questions;
                                //         if(questions[queId]){
                                //             questions[queId].bool = bool;
                                //         }else{
                                //           questions[queId] = {queid:queId,bool:bool}
                                //         }
                                        
                                //         healthInfo.members[mkey].medical = questions;
                                //      }else{
                                //         error++;
                                //         $('.error-yesno-boolean-'+q_uid).text("Please select Yes/No?");
                                //      } 
                                //  }
                                 if(isText && ($('#medi_text_'+q_uid).length>0)){
                                     var text = $('#medi_text_'+q_uid).val();
                                     if(text!==""){
                                        $('.error-desc-text-'+q_uid).text("");
                                        //var questions = {};
                                        // questions = (healthInfo.members[mkey].medical)?healthInfo.members[mkey].medical:questions;
                                        if(questions[queId]){
                                            questions[queId].text = text;
                                        }else{
                                           questions[queId] = {queid:queId,text:text}
                                        }
                                        
                                        healthInfo.members[mkey].medical = questions;
                                     }else{
                                        error++;
                                        $('.error-desc-text-'+q_uid).text("Please enter description.");
                                     } 
                                 }
                            }else{
                                 // console.log("MemberCheck",$(this));
                                  var questions = (healthInfo.members[$(this).attr('data-key')].medical)?healthInfo.members[$(this).attr('data-key')].medical:{};
                                  if(questions[queId]){ delete questions[queId]; }
                            }
                         })
                    }else{
                        error++;
                      ...
                      //console.log("QueCheck",$(this).val());
                      if(questionsArr[$(this).val()]){ delete questionsArr[$(this).val()]; }
                  }
                }); 
            ...
            $(".medical_que").each(function( index ){
                ...
                $(".ins-members-"+queId ).each(function( index ) {
                    var mkey =  $(this).attr('data-key');
                    var questions = {};
                    if(healthInfo.members[mkey].medical){ healthInfo.members[mkey].medical=questions; }
                });
