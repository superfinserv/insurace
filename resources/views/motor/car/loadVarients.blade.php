...
<div class="row mt-3 text-center">
	    ...
	    foreach ($varients as $value) { 
    	     //if($rs==0){ echo'<div class="col-md-1"></div>'; }?>
              <div class="col-md-4 col-sm-4 variant choose-varient" data-animation="fadeInDown" data-cc="{{$value->cubic_capacity}}" data-fule="{{$value->fuel_type}}" data-delay="100" data-name="{{$value->variant}}" data-id="{{$value->id}}">
                    	<a  class="Chevrolet-pro br-all" style="border-radius: 8px;border: 1px solid #AC0F0B;" data-toggle="tooltip" data-placement="top"  title="{{$value->variant}}-{{$value->cubic_capacity}}cc">  
                        	<h4 style="font-weight: 700;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;">{{$value->variant}}<br/>({{$value->fuel_type}} - {{$value->seating_capacity}} Seater)</h4>
