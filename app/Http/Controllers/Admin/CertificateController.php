...
class CertificateController extends Controller{
            ...
            if($request->testData){
                      ...
                      DB::table('certification')
                        ...
                        if($p>=$reqMarks){
                           ...
                           $this->certificateGenrate($agInfo->posp_ID,$cid); 
    ...
    public function regenerate_certificate(Request $request){
        if($request->id!=""){
          ...
          ///print_r($cert);
           ...
           $this->certificateGenrate($agInfo->posp_ID,$cert->id);
        //   $TEMP = getMailSmsInfo($request->id,'CERTIFICATION_COMPLETE');
        //     sendNotification($TEMP);
           $newcert = DB::table('certification')->where('agent_id',$request->id)->orderBy('id', 'desc')->first();
            return response()->json(['status'=>'success','message'=>'Certificate Regerated successfully!','path'=>url('/get/download/file/test-certificate/'.$newcert->file)]);
          }else{
    ...
    public function certificateGenrate($pospID,$id){
        ...
        if(!File::exists($path)) {
         ...
         $pdf = PDF::loadView('admin.agents.certificate_format',$data);
